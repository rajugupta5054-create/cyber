...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    if (topColor) materialRef.current.uniforms.uTopColor.value = parseColor(topColor);
  }, [topColor]);
  ...
  useEffect(() => {
    ...
    if (bottomColor) materialRef.current.uniforms.uBottomColor.value = parseColor(bottomColor);
  }, [bottomColor]);
  ...
  useEffect(() => {
    if (!materialRef.current) return;
    const pillarRotRad = ((pillarRotation || 0) * Math.PI) / 180;
